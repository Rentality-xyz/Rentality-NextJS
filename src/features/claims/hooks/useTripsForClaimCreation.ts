...
const useTripsForClaimCreation = (isHost: boolean) => {
  ...
  const queryResult = useQuery<QueryData>({
    queryKey: [TRIPS_FOR_CLAIM_CREATION_QUERY_KEY, rentalityContracts, isHost, ethereumInfo?.walletAddress],
    queryFn: async () => fetchTripsForClaimCreation(rentalityContracts, isHost),
  });
...
async function fetchTripsForClaimCreation(rentalityContracts: IRentalityContracts | null | undefined, isHost: boolean) {
