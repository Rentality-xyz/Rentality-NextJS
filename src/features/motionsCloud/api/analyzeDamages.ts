...
import { createMotionCloudCase } from "../models";
...
function validateEnvs(chainId: number) {
  const baseUrl = env.NEXT_PUBLIC_MOTIONSCLOUD_BASE_URL;
  if (isEmpty(baseUrl)) {
    return Err(new Error("NEXT_PUBLIC_MOTIONSCLOUD_BASE_URL was not set"));
  }

  const accountSid = env.NEXT_PUBLIC_MOTIONSCLOUD_ACCOUNT_SID;
  if (isEmpty(accountSid)) {
    return Err(new Error("NEXT_PUBLIC_MOTIONSCLOUD_ACCOUNT_SID was not set"));
  }

  const accountSecret = env.NEXT_PUBLIC_MOTIONSCLOUD_ACCOUNT_SECRETKEY;
  if (isEmpty(accountSecret)) {
    return Err(new Error("NEXT_PUBLIC_MOTIONSCLOUD_ACCOUNT_SECRETKEY was not set"));
  }
...
async function getApiAccessToken(
  ...
  if (response.status !== 200) {
    return Err(new Error("MotionsCloud: failed to create secret with error: ", response.data));
  }

  const accessToken = response.data.access_token;

  if (accessToken === undefined || typeof accessToken !== "string") {
    return Err(new Error("MotionsCloud: access_token is missing in response body"));
  }
...
async function createCase(
  ...
  const newCase = createMotionCloudCase(
    request.tripId,
    request.chainId,
    request.fullName,
    request.email,
  ...
  try {
    ...
    if (response.status !== 201) {
      return Err(new Error("MotionsCloud: failed to create case with error: ", response.data));
    }
...
async function saveCasePhotos(
  ...
  if (!tripPhotos) {
    return Err(new Error("MotionsCloud: no pre trip photos to upload"));
  }
  ...
  try {
    ...
    if (response.status !== 200) {
      return Err(new Error("MotionsCloud: failed to upload photo with error: ", response.data));
    }
    logger.debug("response.data:", JSON.stringify(response.data, null, 2));
    return Ok(true);
  } catch (error) {
...
async function saveCaseToBlockchain(
  ...
  const caseExists = await rentalityMotionsCloud.isCaseTokenExists(token);
  if (caseExists) {
    return Err(new Error("MotionsCloud: case exists: " + token));
  }

  try {
    const tx = await rentalityMotionsCloud.saveInsuranceCase(
      token,
      BigInt(tripId),
