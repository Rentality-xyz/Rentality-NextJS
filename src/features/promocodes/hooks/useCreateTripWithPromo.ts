...
const useCreateTripWithPromo = () => {
  const ethereumInfo = useEthereum();
  const rentalityContract = useRentality();

  async function createTripRequestWithPromo(
    carId: number,
    ...
    if (!ethereumInfo) {
      console.error("createTripRequestWithPromo error: ethereumInfo is null");
      return Err("ERROR");
    }

    if (!rentalityContract) {
      console.error("createTripRequestWithPromo error: rentalityContract is null");
      return Err("ERROR");
    }
    ...
    try {
      if (
        ...
        const paymentsNeeded = await rentalityContract.calculatePaymentsWithDeliveryAndPromo(
          BigInt(carId),
          BigInt(days),
          ETH_DEFAULT_ADDRESS,
          pickupLocationInfo,
        ...
        if (!(await isUserHasEnoughFunds(ethereumInfo.signer, totalPriceInEth))) {
          console.error("createTripRequestWithPromocreateTripRequestWithPromo error: user don't have enough funds");
          return Err("NOT_ENOUGH_FUNDS");
        }

        const transaction = await rentalityContract.createTripRequestWithDeliveryAndPromo(tripRequest, promo, {
        ...
        if (!(await isUserHasEnoughFunds(ethereumInfo.signer, totalPriceInEth))) {
          console.error("createTripRequestWithPromo error: user don't have enough funds");
          return Err("NOT_ENOUGH_FUNDS");
        }
        const transaction = await rentalityContract.createTripRequestWithDeliveryAndPromo(tripRequest, promo, {
          value: paymentsNeeded.totalPrice,
      ...
      console.error("createTripRequestWithPromo error:" + e);
      return Err("ERROR");
    }
  }

  return { createTripRequestWithPromo } as const;
};

export default useCreateTripWithPromo;
