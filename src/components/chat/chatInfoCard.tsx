...
export default function ChatInfoCard({ chatInfo, isHost, isSelected, onClickCallback }: Props) {
  ...
  return (
    <div key={chatInfo.tripId} className={className} onClick={onClickCallback}>
      ...
      <div className="col-span-2 self-end font-bold text-lg">{otherName}</div>
      <div className="text-sm whitespace-nowrap overflow-hidden overflow-ellipsis">{chatInfo.tripTitle}</div>
