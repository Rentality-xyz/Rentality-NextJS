      ...
      return async (...args: any[]) => {
        try {
          ...
          const isReadFunction =
            functionFragment &&
            (functionFragment.stateMutability === "view" || functionFragment.stateMutability === "pure");
          ...
          if (!isReadFunction && !isDefaultNetwork) {
            ...
            const value =
              functionFragment && functionFragment.payable && args[args.length - 1].value
            ...
            let quoteResult = await quoteMethod.apply(contractToUse, [value, gasLimit, argmuments]);
            ...
            result = await originalMethod.apply(writeContract, [value, gasLimit, argmuments, { value: quoteResult }]);
          } else {
            const originalMethod = Reflect.get(contractToUse, key, receiver);
