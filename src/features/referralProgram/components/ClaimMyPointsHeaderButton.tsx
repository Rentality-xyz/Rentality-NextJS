...
export default function ClaimMyPointsHeaderButton() {
  ...
  return (
    ...
    // </button>
    <button
      className="ml-[116px] hidden max-w-[260px] items-center rounded-md border border-gray-500 px-4 py-2 hover:border-gray-400 xl:flex"
      disabled={isLoading || isFetching || isClaiming || readyToClaim === 0}
      onClick={() => claimMyPoints()}
      title={`${t("referrals_and_point.claim")} ${readyToClaim.toString()} ${t("referrals_and_point.points")}`}
    >
      <Image src="/images/icons/ic_star_points_yellow.svg" width={47} height={47} alt="" className="mr-2 h-7 w-7" />

      <div className="ml-0.5 min-w-0 flex-1 truncate">
        {isClaiming ? (
          t("referrals_and_point.claiming")
        ) : isLoading || isFetching ? (
          t("referrals_and_point.loading")
