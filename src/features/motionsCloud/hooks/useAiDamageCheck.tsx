...
export const MOTION_CLOUD_QUERY_KEY = "MotionCloudQueryKey";
...
function useAiDamageCheck(tripId: number) {
  ...
  const { mutateAsync: startPreTripCheck } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [MOTION_CLOUD_QUERY_KEY] });
    },
  });
  ...
  const { data } = useQuery<QueryData>({
    queryKey: [MOTION_CLOUD_QUERY_KEY, tripId, rentalityContracts, ethereumInfo?.walletAddress],
    queryFn: async () =>
...
async function fetchAiDamageCheck(
  ...
  const tripCasesResult = await rentalityContracts.motionsCloud.getInsuranceCasesUrlByTrip(BigInt(tripId));
...
async function startDamageAnalyzeImpl(
  ...
  try {
    ...
    const caseInfoResult = await rentalityContracts.gateway.getMotionsCloudCaseData(
      BigInt(tripId),
      type === "pre-trip"
    );
    if (!caseInfoResult.ok) {
      logger.error("startPreTripCheck: failed to get MotionsCloudCaseData with error: ", caseInfoResult.error);
      return Err(new Error("Failed to get MotionsCloudCaseData"));
    }
    ...
    const response = await axios.post("/api/motionscloud/analyzeDamages", body);
    ...
    logger.info("startPostTripsAnalyzeImpl: MotionCloud case created!");
    return Ok(true);
  } catch (error) {
