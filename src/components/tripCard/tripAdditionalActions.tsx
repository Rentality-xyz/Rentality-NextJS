...
function TripAdditionalActions({
  ...
  const handleButtonClick = () => {
    ...
    changeStatusCallback(() => {
      return tripInfo.allowedActions[0].action(BigInt(tripInfo.tripId), inputParams, tripInfo.insuranceCompany, tripInfo.insuranceNumber);
    });
  };
    ...
    return (
        <div className="flex flex-col px-8 pt-2 pb-4">
            ...
            <div className="flex max-md:flex-col">
                <div className="flex flex-col w-full md:w-1/2 3xl:w-1/3" ref={refForScrool}>
                        ...
                        {tripInfo.allowedActions[0].params.map((param, index) => {
                            return (
                                <div className="flex flex-col md:flex-row" key={param.text}>
                                    <div className="flex items-end w-full md:w-10/12">
                                        {param.type === "fuel" ? (
                                            <RntSelect
                                                className="w-full"
                                                id={param.text}
                ...
                <div className="flex flex-col max-md:mt-4 w-full md:w-1/2 3xl:w-1/3">
                    <div id="trip-allowed-actions-insurance">
                        <strong className="text-xl">
                            {t("booked.guest_insurance_information", {
                                type: tripInfo.allowedActions[0].readonly ? "confirm" : "enter",
                            })}
                        </strong>
                    </div>
                    <div className="flex flex-col gap-4 py-4 w-full md:w-10/12">
                        <RntInput
                            label={t("booked.insurance_company_name")}
                            value={tripInfo.insuranceCompany}
                            // onChange={handleSearchInputChange}
                        />
                        <RntInput
                            label={t("booked.insurance_policy_number")}
                            value={tripInfo.insuranceNumber}
                            // onChange={handleSearchInputChange}
                        />
                    </div>
                </div>
                        ...
                        <RntButton
                            ...
                            onClick={() => {
                                if (action.params == null || action.params.length == 0) {
                                    changeStatusCallback(() => {
                                        return action.action(BigInt(tripInfo.tripId), [], tripInfo.insuranceCompany, tripInfo.insuranceNumber);
                                    });
                                } else {
                                    handleButtonClick();
                                }
