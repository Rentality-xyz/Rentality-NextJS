            ...
            <PageTitle title={t_details("title", {tripId: tripId.toString()})} />
                {
                    isLoading ? (
                        <div
                            className="mt-5 flex max-w-screen-xl flex-wrap justify-between text-center">{t("common.info.loading")}</div>
                    ) : (
                        <>
                            <div className="my-4 flex flex-col md:grid grid-cols-2 gap-4 pr-4">
                                <div className="flex flex-row gap-4">
                                    <RntInput id="tripId" label={t_details("tripId")}
                                              value={tripDetails.tripId.toString()}
                                              readOnly={true}/>
                                    <RntInput id="carId" label={t_details("carId")} value={tripDetails.carId.toString()}
                                              readOnly={true}/>
                                </div>
                                <RntInput id="status" label={t_details("status")} value={tripDetails.status}
                                          readOnly={true}/>
                                <RntInput id="host" label={t_details("host_addr")} value={tripDetails.host}
                                          readOnly={true}/>
                                <RntInput id="guest" label={t_details("guest_addr")} value={tripDetails.guest}
                                          readOnly={true}/>
                                <RntInput
                                    id="startDateTime"
                                ...
                                <RntInput
                                    ...
                                    readOnly={true}
                                />
                                <RntInput id="startLocation" label={t_details("start_location")}
                                          value={tripDetails.startLocation}
                                          readOnly={true}/>
                                <RntInput id="endLocation" label={t_details("end_location")}
                                ...
                                <RntInput
                                    ...
                                    readOnly={true}
                                />
                                <RntInput id="paymentFrom" label={t_details("payments", {value: "from"})}
                                          value={tripDetails.paymentFrom}
                                          readOnly={true}/>
                                <RntInput id="paymentTo" label={t_details("payments", {value: "to"})}
