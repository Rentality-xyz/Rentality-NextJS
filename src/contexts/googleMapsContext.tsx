...
export const GoogleMapsProvider = ({
  ...
  useEffect(() => {
    ...
    const existingScriptTag: HTMLScriptElement | null = document.querySelector(`script[src^="${GOOGLE_MAPS_API_URL}"]`);
  ...
  }, [JSON.stringify(libraries), language, region, version, authReferrerPolicy]);

  return (
    <GoogleMapsContext.Provider value={{ googleMapsAPIIsLoaded: !isLoadingAPI }}>{children}</GoogleMapsContext.Provider>
  );
};
