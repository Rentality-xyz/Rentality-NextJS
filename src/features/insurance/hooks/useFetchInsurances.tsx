...
function useFetchInsurances(isHost: boolean, initialPage: number = 1, initialItemsPerPage: number = 10) {
  ...
  const queryResult = usePaginationForListApi<TripInsurance>(
    {
      queryKey: [INSURANCE_LIST_QUERY_KEY, rentalityContracts, isHost, ethereumInfo?.walletAddress],
      queryFn: async () => fetchInsurances(rentalityContracts, isHost),
    },
    initialPage,
    initialItemsPerPage
  );
...
async function fetchInsurances(rentalityContracts: IRentalityContracts | null | undefined, isHost: boolean) {
